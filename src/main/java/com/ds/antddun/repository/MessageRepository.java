...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    // 안읽은 메시지 개수 카운트
    @Query("SELECT COUNT(msg.msgNo) " +
            " FROM Message msg " +
            " WHERE msg.msgRead = 0 " +
            " AND msg.receiveMember.mno=:loginMember " +
            " AND msg.sendMember.mno!=:loginMember ")
    int unreadMsg(@Param("loginMember")Long loginMember);
    ...
    // 거래중 / 거래완료
    @Modifying
    @Query("UPDATE Message msg SET msg.trade=:tradeState WHERE msg.board.sosoNo=:sosoNo AND msg.sendMember.mno=:sendMember ")
